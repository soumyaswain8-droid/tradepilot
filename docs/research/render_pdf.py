"""Render the architecture research HTML to PDF + visual QA."""
import asyncio
import subprocess
import tempfile
...
HTML = HERE / "2026-05-01_independent_engines_research.html"
PDF = HERE / "2026-05-01_independent_engines_research.pdf"
CHROME = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"


async def render():
    user_dir = tempfile.mkdtemp(prefix="tp-research-")
...
def qa():
    ...
    qa_pages = sorted({1, 2, 4, max(1, total // 2), max(1, total - 1), total})
    for p in qa_pages:
        if 1 <= p <= total:
            w = PdfWriter()
            w.add_page(reader.pages[p - 1])
            single = HERE / f"qa_rs_p{p}.pdf"
            with open(single, "wb") as f:
                w.write(f)
            subprocess.run(["qlmanage", "-t", "-s", "900", "-o", str(HERE), str(single)],
                           capture_output=True, timeout=15)
            print(f"  qa_rs_p{p}.pdf.png")
