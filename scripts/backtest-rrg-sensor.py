...
DATA-REPAIR PASS (2026-07-20, bounded, NOT a recalibration): the original
run found `NIFTY_HEALTHCARE.NS` (defensive) and `NIFTYPVTBANK.NS`
(cyclical-extended-only) returning zero bars ("possibly delisted") from
yfinance. A symbol probe (period="1mo", daily; accept only >15 bars covering
July 2026 with plausible thousands-level index closes) found a working
replacement for NIFTY Pvt Bank -- `NIFTY_PVT_BANK.NS` (52 bars,
2026-05-04..2026-07-16, closes ~27,900-28,150) -- now used below. No working
symbol was found for NIFTY Healthcare despite trying `^CNXHEALTH`,
`NIFTYHEALTHCARE.NS`, `NIFTY-HEALTHCARE.NS`, `NIFTY_HEALTHCARE.BO`,
`NIFTY100HEALTHCARE.NS`, `NIFTYHEALTHCARE25.NS`, `NIFTY_HEALTH.NS`,
`^NSEIHEALTHCARE`, `NIFTY_HEALTHCARE25.NS`, `^NIFTYHEALTHCARE`,
`^NSEHEALTHCARE`, `NIFTY_HEALTHCARE` -- all returned no data. Per the
data-repair instruction, NIFTY Healthcare is NOT substituted with a
different index (e.g. Consumption); it remains dead and the defensive set
continues to fall back to its existing fail-closed 2/3-present handling.
Set membership (apart from the one repaired ticker) and threshold-grid
derivation are otherwise unchanged from the original run -- see the
"Data-repair re-run" section appended to the report for the side-by-side
comparison.
...
BENCHMARK = "^NSEI"
DEFENSIVE = ["^CNXPHARMA", "^CNXFMCG", "NIFTY_HEALTHCARE.NS"]  # NIFTY_HEALTHCARE.NS: UNREPAIRABLE (2026-07-20 probe), stays dead
CYCLICAL_BASE = ["^NSEBANK", "^CNXAUTO", "^CNXMETAL", "^CNXREALTY"]
CYCLICAL_EXT_ADD = ["^CNXPSUBANK", "NIFTY_PVT_BANK.NS", "NIFTY_FIN_SERVICE.NS"]  # repaired 2026-07-20: was NIFTYPVTBANK.NS (dead)
...
def main():
    ...
    report.append(
        "Bounded data-repair pass, explicitly NOT a re-calibration: same forms, sets, "
        "lookbacks, and threshold-grid derivation as the original run above (zero new "
        "knobs). A symbol probe (period=\"1mo\", daily; accept only >15 bars covering "
        "July 2026 with plausible thousands-level index closes) found a working "
        "replacement for **NIFTY Pvt Bank** -- `NIFTY_PVT_BANK.NS` (52 bars, "
        "2026-05-04..2026-07-16, closes ~27,900-28,150) -- now substituted in place of "
        "the dead `NIFTYPVTBANK.NS` in the cyclical-extended-only set. **NIFTY Healthcare "
        "remains UNREPAIRABLE**: 12 candidate symbols tried "
        "(`^CNXHEALTH`, `NIFTYHEALTHCARE.NS`, `NIFTY-HEALTHCARE.NS`, "
        "`NIFTY_HEALTHCARE.BO`, `NIFTY100HEALTHCARE.NS`, `NIFTYHEALTHCARE25.NS`, "
        "`NIFTY_HEALTH.NS`, `^NSEIHEALTHCARE`, `NIFTY_HEALTHCARE25.NS`, "
        "`^NIFTYHEALTHCARE`, `^NSEHEALTHCARE`, `NIFTY_HEALTHCARE`), all returned no "
        "data. Per instruction, no substitute index (e.g. Consumption) was used -- the "
        "defensive set keeps falling back to its existing fail-closed 2/3-present "
        "handling, unchanged from the original run. **Only the cyclical-extended set "
        "changes** (6/7 -> 7/7 members present); the base set (which never included the "
        "pvt-bank ticker) is identical to the original run by construction.\n")
    report.append(
        f"**Data-repair best: form={best['form']}, set={best['set']}, N={best['N']}, "
        f"threshold={best['threshold']:.4f} -> profit-capture {best['pc']:.0f}%, "
        f"loss-capture {best['lc']:.0f}% ({verdict} vs 70/70 gate)**\n")

    report.append(
        "Sensor (unchanged from original run): daily close-to-close relative return of each sector index vs ^NSEI "
    ...
    # results -- they stay above the "Data-repair re-run" marker as an audit trail.
    # Re-running this script again is idempotent: any prior repair section (matched
    # by the marker) is dropped and rebuilt fresh, rather than appended twice.
    REPAIR_MARKER = "## Data-repair re-run"
    original_text = out.read_text() if out.exists() else ""
    if REPAIR_MARKER in original_text:
        original_text = original_text.split(REPAIR_MARKER)[0].rstrip("\n") + "\n"
    out.write_text(original_text + "\n".join(report))
    print("\n".join(report[:3]))
    print(f"\nData-repair best: form={best['form']} set={best['set']} N={best['N']} "
