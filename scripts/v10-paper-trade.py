#!/usr/bin/env python3
"""
v10 — FROZEN April-2026 engine. Vendored verbatim from git 9d7db34 (2026-04-16).

WHY: v8 claimed to be the "April replica" but runpy's into TODAY's 1421-line
v5 engine with April parameters as env vars — it tests today's code wearing
April's settings, not April's code. v10 is the actual April engine: the 732-line
orchestrator plus the only two signal-path modules that drifted since
(signal_engine +51 lines, risk_manager +194). The other six modules on that path
are byte-identical to April and are imported from prototype.v5 unchanged.

EXACTLY TWO DIVERGENCES from git 9d7db34, both mandatory, neither behavioural:
  1. TRADE_DIR / LOG_FILE: "v5" -> "v10". The original hardcodes v5 and would
     overwrite LIVE v5 state. Data safety, not strategy.
  2. _mod_imports "signals" and "risk" point at prototype.v10.*, the frozen
     April copies. Without this v10 would import today's picker and the
     experiment would be meaningless.
Nothing else is touched. Costs are deliberately NOT booked (April did not book
...
TRADE_DIR = PROJECT_ROOT / "docs" / "paper-trades" / "v10"
LOG_DIR = PROJECT_ROOT / "logs"
sys.path.insert(0, str(PROJECT_ROOT / "prototype"))
sys.path.insert(0, str(PROJECT_ROOT))
LOG_FILE = LOG_DIR / "v10-paper-trade.log"
...
_mod_imports = {
    ...
    "signals": ("prototype.v10.signal_engine", "generate_signals"),
    "risk": ("prototype.v10.risk_manager", "RiskManager"),
}
