#!/usr/bin/env python3
"""
TradePilot v5.5 Paper Trading Engine — Regime-Aware Exit Logic
================================================================
Based on v5 but with smarter SIGNAL_FLIP behavior:
  BULL:     Only exit on SELL signal (HOLD = keep position)
  SIDEWAYS: Exit on SELL or HOLD (same as v5)
  BEAR:     Exit on SELL or HOLD (same as v5)

Also: Carry-forward positions get a grace period (skip first rescore)

This fixes v5's Day 5 problem: 51% of trades exited via SIGNAL_FLIP
in BULL regime because rankings shifted (not because stocks were bad).

Usage:
    python3 scripts/v5_5-paper-trade.py              # Full auto-pilot
    python3 scripts/v5_5-paper-trade.py --status      # All pools + positions
    python3 scripts/v5_5-paper-trade.py --summary     # P&L summary
"""
...
TRADE_DIR = PROJECT_ROOT / "docs" / "paper-trades" / "v5_5"
LOG_DIR = PROJECT_ROOT / "logs"
sys.path.insert(0, str(PROJECT_ROOT / "prototype"))
sys.path.insert(0, str(PROJECT_ROOT))
LOG_FILE = LOG_DIR / "v5_5-paper-trade.log"
...
CARRY_FORWARD_FILE = TRADE_DIR / "carry_forward_v5_5.json"
...
def fresh_state(capital=None):
    ...
    return {"date": datetime.now().strftime("%Y-%m-%d"), "engine": "v5.5-regime-flip",
...
def print_status(state):
    ...
    R = "\033[0m"
    print(f"\n{'='*65}")
    print(f"  v5.5 REGIME-FLIP  |  {state.get('date','today')}  |  Capital: {_fmt(TOTAL_CAPITAL)}")
    ...
    try:
        ...
        cur.execute(
            ...
            ("tradepilot", "paper-trade",
             ...
             json.dumps({"engine": "v5.5-regime-flip", "capital": TOTAL_CAPITAL, "regime": state.get("regime", "?"),
...
def run():
    log(f"{'='*65}\n  v5.5 REGIME-FLIP ENGINE | {_fmt(TOTAL_CAPITAL)} | Multi-Pool + Short\n"
