#!/usr/bin/env python3
"""
TradePilot v5.7 Paper Trading Engine — Intraday Box Theory (Mean Reversion)
=============================================================================
OPPOSITE of v5.6 (Darvas breakout). This engine uses previous day's box:

  Box = Previous day's HIGH (ceiling) and LOW (floor)

  Price near BOTTOM of box + green candle confirmation → BUY
  Price near TOP of box + red candle confirmation → SELL (short)
  Price in MIDDLE → DO NOTHING

This is a MEAN REVERSION strategy — it works best in SIDEWAYS markets
where stocks bounce between yesterday's high and low.

v5.6 (Darvas) = breakout/trend following = BULL markets
v5.7 (Box)    = mean reversion          = SIDEWAYS markets

Uses v5.5 regime-flip exit logic + box-based entry/exit.

Usage:
    python3 scripts/v5_7-paper-trade.py              # Full auto-pilot
    python3 scripts/v5_7-paper-trade.py --status
    python3 scripts/v5_7-paper-trade.py --summary
"""
...
TRADE_DIR = PROJECT_ROOT / "docs" / "paper-trades" / "v5_7"
...
CARRY_FORWARD_FILE = TRADE_DIR / "carry_forward_v5_7.json"
...
def fresh_state(capital=None):
    ...
    return {"date": datetime.now().strftime("%Y-%m-%d"), "engine": "v5.7-intraday-box",
...
def deploy_signals(state, pm, rm, signals):
    """v5.7: Filter signals through intraday box theory before deploying."""
    ...
    # ═══ v5.7: INTRADAY BOX THEORY OVERLAY ═══
    # For each signal, check the box position. Only deploy if:
    # - BUY signal AND price is near box BOTTOM (buy the dip)
    # - SELL signal AND price is near box TOP (sell the rip)
    # - If box says opposite of signal → SKIP (conflicting)
    # - If box says MIDDLE → deploy with reduced size (no edge)
    try:
        from prototype.v5_7.intraday_box import check_box_signal
        box_available = True
        log("  [BOX] Intraday Box Theory active")
    except ImportError:
        box_available = False
    ...
    for sig in sorted([s for s in signals if s["direction"] in ("BUY", "SELL")],
        ...
        if box_available:
            try:
                box_result = check_box_signal(sym)
                box_sig = box_result.get("signal", "NONE")
                box_zone = box_result.get("zone", "UNKNOWN")
                box_score = box_result.get("score", 0)

                # Case 1: ML says BUY and Box says BUY (at bottom) → STRONG ENTRY
                if sig["direction"] == "BUY" and box_sig == "BUY" and box_result.get("confirmed"):
                    sl = box_result.get("sl_price", sl)  # Use candle-based SL
                    tgt = box_result.get("target_price", tgt)  # Target = box ceiling
                    qty = max(1, int(qty * 1.3))  # 30% boost — double conviction
                    box_boosted += 1
                    log(f"  [BOX] {sym}: ML+BOX aligned BUY at bottom (score={box_score}) SL={sl:.2f} TGT={tgt:.2f}")

                # Case 2: ML says BUY but Box says near TOP → CONFLICT, skip
                elif sig["direction"] == "BUY" and box_zone == "TOP":
                    box_filtered += 1
                    log(f"  [BOX] {sym}: ML says BUY but price at box TOP — SKIP (mean reversion conflict)")
                    continue

                # Case 3: ML says SELL and Box says SELL (at top) → SHORT entry
                elif sig["direction"] == "SELL" and box_sig == "SELL" and box_result.get("confirmed"):
                    sl = box_result.get("sl_price", sl)
                    tgt = box_result.get("target_price", tgt)
                    qty = max(1, int(qty * 1.3))
                    box_boosted += 1
                    log(f"  [BOX] {sym}: ML+BOX aligned SELL at top (score={box_score}) SL={sl:.2f} TGT={tgt:.2f}")

                # Case 4: ML says SELL but Box says near BOTTOM → CONFLICT, skip
                elif sig["direction"] == "SELL" and box_zone == "BOTTOM":
                    box_filtered += 1
                    log(f"  [BOX] {sym}: ML says SELL but price at box BOTTOM — SKIP")
                    continue

                # Case 5: Price in MIDDLE of box → reduce size (no edge)
                elif box_zone == "MIDDLE":
                    qty = max(1, int(qty * 0.7))  # 30% smaller — no box edge
                    log(f"  [BOX] {sym}: middle of box — reduced size")

                # Case 6: Price ABOVE or BELOW box → let ML decide (breakout/breakdown)
                # No filter applied, proceed normally

            except Exception as e:
                pass  # Box check failed, proceed with ML signal only
    ...
    if count or box_filtered:
        box_note = f" | Box: {box_boosted} boosted, {box_filtered} filtered" if box_available else ""
        log(f"  Deployed {count} positions (v5.7 Box Theory){box_note}")
    return count
...
def print_status(state):
    ...
    R = "\033[0m"
    print(f"\n{'='*65}")
    print(f"  v5.7 INTRADAY-BOX  |  {state.get('date','today')}  |  Capital: {_fmt(TOTAL_CAPITAL)}")
    ...
    try:
        ...
        cur.execute(
            ...
            ("tradepilot", "paper-trade",
             ...
             json.dumps({"engine": "v5.7-intraday-box", "capital": TOTAL_CAPITAL, "regime": state.get("regime", "?"),
